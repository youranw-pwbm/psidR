...
#' @param wealth.vars data.frame of variables to get from the 1984-1994 wealth supplement files.
...
medium.test.ind.NA.wealth <- function(dd=NULL){
	...
	build.panel(fam.vars=famvars,ind.vars=indvars,wealth.vars
...
build.psid <- function(datadr="~/datasets/psid/",small=TRUE,wealth=FALSE){
  ...
  if (wealth) {
    w = dcast(w[,list(year,name,variable)],year~name)
    d = build.panel(datadir=datadr,fam.vars=f,ind.vars=i,wealth.vars
...
getNamesPSID <- function(aname, cwf, years = NULL, file = NULL){
    ...
    # distinguish the year cols in cwf (those with names that begin with Y)
